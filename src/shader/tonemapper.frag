...
layout (location = 1) out vec4 log_luma;
...
void main() {
    ...
    vec3 hdrColor = tonemapWhitePreservingLuma(color);
    ...
    float luma_r = color.r * 0.2126;
    float luma_g = color.g * 0.7152;
    float luma_b = color.b * 0.0722;
    log_luma = vec4(vec3(log2(luma_r + luma_g + luma_b)), 1.0);
}
