...
use crate::memory::xalloc::{XallocMemoryPoolBlock, XallocMemoryPool, get_global_pool};
...
impl<T> CpuAccessibleBufferXalloc<T> {
        ...
        where T: Content + 'static
    {
        unsafe {
            let uninitialized =
                CpuAccessibleBufferXalloc::raw(device, mem::size_of::<T>(), usage, iter::empty())?;
...
impl<T> CpuAccessibleBufferXalloc<[T]> {
              ...
              T: Content + 'static
    {
        unsafe {
            let uninitialized =
                CpuAccessibleBufferXalloc::uninitialized_array(device, data.len(), usage)?;
    ...
    pub unsafe fn uninitialized_array(
        ...
        -> Result<Arc<CpuAccessibleBufferXalloc<[T]>>, DeviceMemoryAllocError> {
        CpuAccessibleBufferXalloc::raw(device, len * mem::size_of::<T>(), usage, iter::empty())
    }
}
...
impl<T: ?Sized> CpuAccessibleBufferXalloc<T> {
        ...
        where I: IntoIterator<Item = QueueFamily<'a>>
    {
        ...
        let pool = get_global_pool(device.clone());
